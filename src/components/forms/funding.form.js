...
export const FundingForm = (props) => {
  ...
  const [funding, setFunding] = useState(
    localStorage.getItem("postIssue")
      ? parseInt(JSON.parse(localStorage.getItem("postIssue")).prize)
      : ""
  );
  ...
  const handleSubmit = async () => {
    ...
    if (funding === undefined || funding === null || funding === 0) {
      ...
      toast.error(
        "Payment Failed: Please Provide the name on your credit card"
      );
      return;
    }
  ...
  return (
    <div hidden={props.value !== props.index} className="fundingForm">
      ...
      <Button classes={{ root: "payButton" }} onClick={share}>
        Can't Fund? Share instead
      </Button>
    </div>
  );
};
