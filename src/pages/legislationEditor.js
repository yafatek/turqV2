...
class LegislationEditor extends React.Component {
  ...
  componentDidMount() {
    const config = {
      ...
      databaseURL:
        "https://turq-legislation-editor-1-default-rtdb.firebaseio.com",
  ...
  _onReady() {
    ...
    if (this.firepad.getHtml() === "") {
      this.firepad.setHtml(
        "<div><b>Legislation Title</b></div><div><b></b><br/></div><div><b>Section 1</b></div><div><b></b><br/></div><div><b>Section 2&nbsp;</b></div><div><b></b><br/></div><div><b>Etc.</b></div>"
      );
    }
    this.setState({ ...this.state, content: text });
  }

  // Update the text stored in state everytime the user changes and text and the
  // db syncs
  _onSynced(isSynced) {
    if (isSynced) {
      const text = this.firepad.getText();
  ...
  _handleSubmit() {
    ...
    const contest = new URLSearchParams(this.props.location.search).get(
      "contest"
    );
    const legislationId = this.props.match.params.id;
    const data = {
      title: "Submitted Bill",
      ref: this.state.ref,
      contestId: contest,
        ...
        {!this.props.isFetching ? (
          <div>
            ...
            <div className="row">
              <div className="col">
                <div
                  dangerouslySetInnerHTML={{ __html: this.state.diffedContent }}
                />
              </div>
            </div>
          </div>
